...
// Vectors a, b, c and d are const. Vector f (the solution vector) is non-const                                                                                                                                                      
// and thus will be calculated and updated by the function.                                                                                                                                                          
void algorithm(const std::vector<double>& a,
    ...
    std::vector<double> c_s(N, 0.0);
    std::vector<double> d_s(N, 0.0);
    ...
    // Create the c_s and d_s coefficients in the forward sweep                                                                                                                                                  
    for (int i = 1; i < N; i++) {
        double m = 1.0 / (b[i] - a[i] * c_s[i - 1]);
        c_s[i] = c[i] * m;
        d_s[i] = (d[i] - a[i] * d_s[i - 1]) * m;
    }
...
// The main function below uses a tridiagonal system from                                                                                                                                                   
